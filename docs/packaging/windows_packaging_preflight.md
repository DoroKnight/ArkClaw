...
Production code under `src/arkclaw` contains only production credential
mapping behavior:

- the legacy OpenAI Target remains stable;
- the built-in DeepSeek credential retains its stable generic Target;
- validated user CredentialIds map to `ArkClaw/Credentials/<uuid>`;
...
calls only `arkclaw.presentation.qt.pet_application.run`. The deployment spec
fixes:

- application title `ArkClaw`;
- standalone mode and Nuitka 4.0;
- Windows console mode `disable`;
- final executable directory `dist`;
- MSVC selection `14.4` and disabled `ccache`;
- executable name `ArkClaw.exe`;
...
execution directory, producing `dist/ArkClaw.dist`. This avoids relying on
...
ArkClaw brand asset and must not be presented as the final product icon.
...
may not bundle it with another product. ArkClaw therefore treats the tool only
as a local build-time dependency and does not claim redistribution permission.
The final ArkClaw package must not contain the ZIP, `depends.exe`,
...
-c
-otD:\ArkClaw\build\dependency-walker-smoke\probe.depends
-d:D:\ArkClaw\build\dependency-walker-smoke\probe.dwp
-f1
-pa1
-ps1
D:\ArkClaw\build\dependency-walker-smoke\probe.exe
```
...
`pyside6-deploy`, ArkClaw production executable, Provider, Credential Manager,
...
`dist/ArkClaw.dist`, an AMD64 GUI `ArkClaw.exe`, exact raw-to-final file and
...
Even after a successful build and static audit, `ArkClaw.exe` remains
...
execution was not observed. No `ArkClaw.exe` was generated or executed, so PE,
...
`arkclaw.infrastructure.llm.openai_sdk` module raised a `NameError` while
...
`arkclaw.infrastructure.llm.openai_sdk` from the current production entry
...
Neither the corrected `ArkClaw.exe` nor any packaged DLL or PYD was executed,
...
was accessed. Neither the new `ArkClaw.exe` nor any packaged DLL or PYD was
