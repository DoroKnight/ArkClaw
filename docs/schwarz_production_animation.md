...
The production launcher is the existing `sjtuclaw.presentation.qt.pet_application`
...
```powershell
$env:SJTUCLAW_PET_ROLE_MANIFEST = 'D:\ArkModels\Schwarz\schwarz.json'
$env:SJTUCLAW_SPINE38_BRIDGE_DLL = `
  'D:\SJTUClaw\build\spine38\Release\sjtuclaw_spine38_bridge.dll'
```
...
12 poses from each of the six physical animations and constructs one immutable
session framing transform from their union.
