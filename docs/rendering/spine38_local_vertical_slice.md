...
This diagnostic exercises one explicit, local-only rendering path for the approved original Schwarz Spine 3.8 asset triplet. It is not selected by normal ArkClaw startup and does not enable the production Agent or action-sequencing path.
...
The copied license is the Spine Runtimes License Agreement from the pinned upstream source. The Runtime is not MIT-, Apache-, BSD-, or ArkClaw-owned code. Runtime source, DLLs, PDBs, generated evidence, and external character assets stay under ignored/local paths and are not packaged by this slice.
...
.venv\Scripts\python.exe scripts\qt_spine38_vertical_slice.py `
  --list-only `
  --bridge-dll "build\spine38\Release\arkclaw_spine38_bridge.dll" `
...
### Fixed visible-frame policy

The Runtime setup bounds remain catalog evidence only; they do not define the Qt viewport framing. After the exact case-sensitive `Relax` animation is bound on track 0, the renderer evaluates it once with a zero-second delta and frames the Qt viewport from the resulting visible draw commands. Commands that are fully transparent are excluded from this envelope. Commands with mixed vertex alpha are retained as whole commands, so their geometry cannot be partially cropped by the framing step.

The logical viewport remains exactly `160 x 180` pixels. The computed `Relax` transform is fixed once at initialization: it uses a 4-pixel top margin, maps the visible feet to the `y = 176` baseline, and is never recomputed while `Relax` loops. This leaves the intended 4-pixel bottom safety gap, prevents scale or position pumping between frames, and keeps the existing default renderer, Agent/action-runtime isolation, one-window behavior, and safe placeholder fallback unchanged.

The opt-in smoke's real-pixel acceptance checks require every sampled nontransparent frame to be at least 80 pixels wide and 150 pixels high, with its bottom edge in the inclusive range `[172, 180]`. These automated checks confirm measured placement and scale, but they remain evidence only: the emitted status stays `visual_review_required` until a person completes the manual checkpoint below.
...
$env:ARKCLAW_SPINE38_ASSET_ROOT = "D:\path\to\approved\runtime_input"
