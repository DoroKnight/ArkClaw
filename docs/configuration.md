...
| 字段 | 默认值 | 环境变量 | CLI 参数 |
|---|---:|---|---|
| `provider` | `fake` | `SJTUCLAW_PROVIDER` | `--provider` |
| `openai_model` | `gpt-5-mini` | `SJTUCLAW_OPENAI_MODEL` | `--openai-model` |
| `deepseek_model` | `deepseek-v4-flash` | `SJTUCLAW_DEEPSEEK_MODEL` | `--deepseek-model` |
| `ollama_model` | `qwen3` | `SJTUCLAW_OLLAMA_MODEL` | `--ollama-model` |
| `provider_timeout_seconds` | `30` | `SJTUCLAW_PROVIDER_TIMEOUT_SECONDS` | `--provider-timeout-seconds` |
| `max_turn_seconds` | `90` | `SJTUCLAW_MAX_TURN_SECONDS` | `--max-turn-seconds` |
| `openai_max_retries` | `2` | `SJTUCLAW_OPENAI_MAX_RETRIES` | `--openai-max-retries` |
| `deepseek_max_retries` | `0` | `SJTUCLAW_DEEPSEEK_MAX_RETRIES` | `--deepseek-max-retries` |
| `ollama_base_url` | `http://127.0.0.1:11434` | `SJTUCLAW_OLLAMA_BASE_URL` | `--ollama-base-url` |
| `max_output_tokens` | `1024` | `SJTUCLAW_MAX_OUTPUT_TOKENS` | `--max-output-tokens` |
| `stream` | `true` | `SJTUCLAW_STREAM` | `--stream` / `--no-stream` |
...
```powershell
.\.venv\Scripts\sjtuclaw-agent-demo.exe --no-stream
```

人工运行 OpenAI Provider：

```powershell
.\.venv\Scripts\sjtuclaw-agent-demo.exe --provider openai
```
...
```powershell
.\.venv\Scripts\sjtuclaw-agent-demo.exe --provider deepseek
```
...
[DeepSeek Provider](deepseek_provider.md)。

## API Key 边界

`RuntimeConfig` 不包含 API Key。项目也没有提供 `--api-key` 参数或
`SJTUCLAW_OPENAI_API_KEY` 普通配置字段。
...
可以显式启用仅操作固定测试 Target `SJTUClaw/Test/OpenAI/APIKey` 的集成测试：

```powershell
$env:SJTUCLAW_RUN_WINDOWS_CREDENTIAL_INTEGRATION = "1"
.\.venv\Scripts\python.exe -m pytest -p no:cacheprovider tests\integration\test_windows_credential_store_integration.py
Remove-Item Env:SJTUCLAW_RUN_WINDOWS_CREDENTIAL_INTEGRATION
```
...
设计依据和限制见 [OpenAI Provider](openai_provider.md)。

多 Provider 的标识、Profile、CredentialId、注册表与无回退约束见
[Provider 架构](provider_architecture.md)。DeepSeek 的请求映射、流终止语义、
continuation 和当前能力限制见 [DeepSeek Provider](deepseek_provider.md)。
