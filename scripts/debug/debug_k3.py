...
# instrument coordinator.eventFilter
orig_filter = coordinator.eventFilter
def spy_filter(watched, event):
    if event.type() in (QEvent.Type.MouseButtonPress, QEvent.Type.MouseButtonRelease):
        log.append(("COORD", str(event.type()).split(".")[-1], str(event.button()), type(watched).__name__))
    return orig_filter(watched, event)
coordinator.eventFilter = spy_filter  # type: ignore[method-assign]
...
def schwarz_pt():
    ...
    return ctypes.wintypes.POINT(rect.left + round(bc.x()*nw/overlay.width()), rect.top + round(bc.y()*nh/overlay.height()))

def epoch_name():
    e = composition.track0.state.confirmed_epoch
    return e.physical_name if e else None
...
print("palette open, visible:", host.isVisible(), "events:", log)
