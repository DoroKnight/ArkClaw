"""Dashboard Character Animation page (Slice 7E).
...
and emits narrow selection intents.  "Active Character" is the frozen product
term; "Schwarz" may only surface as the reference character.  Animation
inventory is capability-driven and unsupported / trigger-unavailable cards
carry a readable disabled reason.  The preview is a labeled placeholder until
the real Spine presentation seam is available ("Visual placeholder"); the
switch crossfade is 180 ms, cancelable, and degrades to the 60 ms reduced
motion crossfade - never semantic truth.
"""
...
class _AnimationCard(QFrame):
    """One capability-driven animation card (frozen 168 x 104)."""
    ...
    def __init__(
        ...
        buttons.addWidget(self._preview)
        buttons.addWidget(self._play)
        buttons.addWidget(self._trigger)
...
class CharacterAnimationPage(QWidget):
    """Frozen Character Animation layout: header, selector, preview, inventory."""
    ...
    def __init__(
        ...
        outer.setSpacing(self._preview_gap)
        ...
        self._body.addLayout(self._character_grid)

        # -- Spine Preview ------------------------------------------------------
        self._preview_frame = _PreviewFrame(self._tokens, self)
        ...
        retry_row.addStretch(1)
        preview_layout.addLayout(retry_row)
        self._body.addWidget(self._preview_frame)

        # -- Preview controls (gap 24 below preview, height 44) ----------------
        self._control_strip = QWidget(self)
        ...
        strip_layout.addStretch(1)
        self._body.addWidget(self._control_strip)

        # -- Animation Selector ---------------------------------------------------
        self._inventory_title = QLabel(
            "Animations", self
        )
        self._inventory_title.setObjectName("sectionTitle")
        self._body.addWidget(self._inventory_title)
        self._inventory_empty = QLabel(
            "No animations available for this character", self
        )
        self._inventory_empty.setObjectName("textSecondary")
        self._inventory_empty.setVisible(False)
        self._body.addWidget(self._inventory_empty)
        self._animation_grid = QGridLayout()
        self._animation_grid.setContentsMargins(0, 0, 0, 0)
        self._animation_grid.setHorizontalSpacing(self._grid_gap)
        self._animation_grid.setVerticalSpacing(self._grid_gap)
        self._animation_grid.setAlignment(
            Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop
        )
        self._body.addLayout(self._animation_grid)
    ...
    def _apply_header(self, snapshot: CharacterAnimationSnapshot) -> None:
        ...
        self._header_name.setText(summary.display_name or "Active Character")
        reference = (
    ...
    def _rebuild_animations(
        ...
        for column, item in enumerate(snapshot.animations):
            card = _AnimationCard(self._tokens, item, self)
            card.preview_requested.connect(
                self.animation_preview_requested
            )
            card.play_requested.connect(self.animation_play_requested)
            card.trigger_requested.connect(
                self.animation_trigger_requested
            )
            self._animation_grid.addWidget(card, 0, column)
            card.show()
    ...
    def cancel_switch_crossfade(self) -> None:
        ...
        # setGraphicsEffect(None) deletes the installed effect, so we only
        # clear the Python reference afterwards (no double-delete).
