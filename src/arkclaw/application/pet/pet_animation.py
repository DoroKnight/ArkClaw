...
from arkclaw.application.pet.pet_motion import PetMotionModel, PetMotionSnapshot
...
from arkclaw.application.pet.pet_role_pack import production_track0_action
...
class PetAnimationEngine:
    ...
    def __init__(
        ...
        *,
        ...
        use_relax_motion_fallback: bool = False,
    ) -> None:
        self._motion = motion
        ...
        self._autonomous_resume_action: ProductionAction | None = None
        ...
        self._use_relax_motion_fallback = use_relax_motion_fallback
    ...
    def resume_autonomous(self, source: ActionSource) -> ActionOutcome:
        """Return Track 0 ownership to the autonomous controller immediately."""
        ...
        resume_action = self._autonomous_resume_action
        ...
        if track0.active_request is not None:
            cleared = track0.clear(CancelReason.USER_INTERRUPT)
            if cleared is not ActionOutcome.CLEARED:
        ...
        if outcome is ActionOutcome.ACCEPTED:
            self._activate_autonomous(resume_action)
        else:
            self._execution_mode = AutonomousExecutionMode.SUSPENDED
        return outcome
    ...
    def _submit_production_intent(
        self,
        intent: ActionIntent,
        *,
        facing_override: PetFacing | None = None,
    ) -> ActionOutcome:
        track0 = self._track0
        if track0 is None:
            return ActionOutcome.LEGACY_DIRECT

        direct_human_ui_override = (
            self._active_request_is_protected()
            and intent.origin is ActionOrigin.EXPLICIT
            and intent.source in {ActionSource.USER, ActionSource.TRAY}
        )
        if (
            direct_human_ui_override
        ...
        if self._active_request_is_protected() and not direct_human_ui_override:
        ...
        proposal = self._motion.states.propose(
            ...
            facing=facing_override or target.facing,
        )
        ...
        decision = (
            ...
            if direct_human_ui_override
            else track0.arbitrate(
                request,
                ArbitrationContext(
        ...
        if intent.action in _PRODUCTION_PROTECTED_ACTIONS:
            ...
            self._execution_mode = AutonomousExecutionMode.AUTONOMOUS
            self._autonomous_resume_action = intent.action
        self._assert_transaction_compatible()
        return outcome
    ...
    def _activate_autonomous(self, action: ProductionAction) -> None:
        ...
        self._execution_mode = AutonomousExecutionMode.AUTONOMOUS
        if action not in _PRODUCTION_PROTECTED_ACTIONS:
            self._autonomous_resume_action = action
    ...
    def handle_playback_event(
        ...
        event: PlaybackEvent,
        *,
        special_completion_facing: PetFacing | None = None,
    ) -> ActionOutcome:
        ...
        if event.loop_boundary and event.boundary_index is not None:
            autonomous_outcome = self._handle_autonomous_boundary(event)
            if autonomous_outcome is not None:
                self._assert_transaction_compatible()
                return autonomous_outcome
        ...
        completed_special_facing = (
            special_completion_facing
            if completes_production_protected
            and self._active_production_action is ProductionAction.SPECIAL
            else None
        )
        ...
        if outcome is ActionOutcome.ACCEPTED and completes_production_protected:
            ...
            if pending is not None:
                ...
                outcome = self._recover_autonomous_relax(
                    facing=completed_special_facing,
                )
    ...
    def _handle_autonomous_boundary(
        self,
        event: PlaybackEvent,
    ) -> ActionOutcome | None:
        scheduler = self._autonomous_scheduler
        state = self._autonomous_scheduler_state
        track0 = self._track0
        ...
        decision = scheduler.evaluate(
            ...
            AutonomousBoundaryEvent(
                ...
                playback_token=event.playback_token,
                boundary_index=boundary_index,
                observed_at=now,
            ),
            self._rng,
        )
        self._autonomous_scheduler_state = decision.state
        if decision.intent is None:
            return track0.handle_completion(event)

        outcome = self._submit_production_intent(decision.intent)
        confirmed = track0.state.confirmed_epoch
        if outcome is ActionOutcome.ACCEPTED and confirmed is not None:
            self._autonomous_scheduler_state = scheduler.commit_accepted(
                decision,
                committed_at=now,
                playback_generation=confirmed.generation,
                playback_token=confirmed.playback_token,
            ...
            self._autonomous_scheduler_state = scheduler.reject(
                decision,
                rejected_at=now,
                rng=self._rng,
            )
    ...
    def _propose_event(
        ...
        if event.event_type is PetAnimationEventType.START_DRAGGING:
            return (
                ...
                (
                    SequenceName.PRODUCTION_MOTION_FALLBACK
                    if self._use_relax_motion_fallback
                    else SequenceName.DRAG_HOLD
                ),
                CancelReason.USER_INTERRUPT,
            )
    ...
    def advance(
        ...
        previous_motion = self._motion.state.motion
        motion = self._motion.update(applied, workspaces)
        if (
            self._use_relax_motion_fallback
            and previous_motion is PetMotionState.LANDING
            and motion.state.motion is PetMotionState.IDLE
        ):
            self._finish_relax_motion_recovery()
        ...
        if direction_turn is not None:
            if self._active_production_action in {
                ...
                if self._execution_mode is AutonomousExecutionMode.EXPLICIT_HOLD:
                    self._recover_explicit_boundary()
                else:
    ...
    def _recover_explicit_boundary(self) -> ActionOutcome:
        self._clear_protected_continuation()
        self._execution_mode = AutonomousExecutionMode.SUSPENDED
        self._active_production_action = None
        track0 = self._track0
        if track0 is None:
            return ActionOutcome.LEGACY_DIRECT
        cleared = track0.clear(CancelReason.MOTION_OVERRIDE)
        if cleared is not ActionOutcome.CLEARED:
            return cleared
        return self._recover_autonomous_relax()
    ...
    def release_drag(
        ...
        if self._production_sequencing_enabled:
            ...
            if self._use_relax_motion_fallback:
                ...
                self._execution_mode = AutonomousExecutionMode.SUSPENDED
                self._active_production_action = None
                self._motion.release_drag(workspaces)
                track0 = self._track0
                if (
                    track0 is None
                    or track0.state.health is not PlaybackHealth.HEALTHY
                    or track0.state.confirmed_epoch is None
                    or track0.state.desired_action is not PetActionName.IDLE
                ):
                    return ActionOutcome.PLAYBACK_DEGRADED
                self._assert_transaction_compatible()
                return ActionOutcome.ACCEPTED
            outcome = self.handle_event(
    ...
    def _finish_relax_motion_recovery(self) -> ActionOutcome:
        track0 = self._track0
        if track0 is None:
            return ActionOutcome.LEGACY_DIRECT
        if (
            track0.state.health is not PlaybackHealth.HEALTHY
            or track0.state.confirmed_epoch is None
            or track0.state.desired_action is not PetActionName.IDLE
        ):
            self._execution_mode = AutonomousExecutionMode.SUSPENDED
            return ActionOutcome.PLAYBACK_DEGRADED
        # Drag/fall/landing already use the confirmed looping Relax epoch.
        # Adopt it as autonomous IDLE instead of clearing and replaying time 0.
        entry = SEQUENCE_CATALOG[SequenceName.PRODUCTION_RELAX]
        adopted = track0.adopt_active_playback(
            ActionRequest(
                sequence_name=SequenceName.PRODUCTION_RELAX,
                interruption_class=entry.interruption_class,
                protected=entry.protected,
                request_token=object(),
                semantic_epoch=self._motion.states.epoch,
        ...
        if adopted is not ActionOutcome.ACCEPTED:
            self._execution_mode = AutonomousExecutionMode.SUSPENDED
            return adopted
        self._active_production_action = ProductionAction.RELAX
        self._activate_autonomous(ProductionAction.RELAX)
