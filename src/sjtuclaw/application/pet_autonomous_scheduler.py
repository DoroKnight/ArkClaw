        ...
        AutonomousState.RELAX: MappingProxyType(
            {
                ...
                AutonomousState.MOVE_LEFT: 10,
                AutonomousState.MOVE_RIGHT: 10,
...
class AutonomousActionScheduler:
    ...
    def evaluate(
        ...
        if dwell is None or event.observed_at < state.entered_at + dwell:
    ...
    @staticmethod
    def reject(decision: AutonomousSchedulerDecision) -> AutonomousSchedulerState:
        if decision.proposed_state is None:
            raise ValueError("decision has no proposed destination")
        return replace(
            decision.state,
            dwell_target_seconds=None,
            proposal_eligible=False,
        )
    ...
    def _select_destination(
        ...
        available = tuple(
            ...
            if weight > 0 and _ACTION_BY_STATE[destination] in capabilities
        )
