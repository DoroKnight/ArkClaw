...
from sjtuclaw.config.errors import ConfigError, SecretStoreError
from sjtuclaw.config.loader import ConfigLoader
from sjtuclaw.config.models import ProviderName, RuntimeConfig
from sjtuclaw.domain.events import AgentEventType
...
async def _run_demo(config: RuntimeConfig) -> None:
    ...
    try:
        ...
        print(f"SJTUClaw Agent Runtime demo ({provider.name} Provider)")
